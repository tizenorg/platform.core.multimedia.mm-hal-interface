...
typedef enum camera_error {
	...
	CAMERA_ERROR_INVALID_ARGUMENT       = 0x80002002,
	CAMERA_ERROR_INVALID_STATE          = 0x80002003,
	CAMERA_ERROR_PERMISSION_DENIED      = 0x80002004,
	CAMERA_ERROR_OUT_OF_MEMORY          = 0x80002005,
	CAMERA_ERROR_DEVICE_OPEN            = 0x80002006,
	CAMERA_ERROR_DEVICE_NOT_FOUND       = 0x80002007,
	CAMERA_ERROR_DEVICE_UNAVAILABLE     = 0x80002008,
	CAMERA_ERROR_DEVICE_NOT_SUPPORTED   = 0x80002009,
	CAMERA_ERROR_DEVICE_ESD             = 0x8000200a,
	CAMERA_ERROR_INTERNAL               = 0x8000200b,
...
typedef enum camera_pixel_format {
	...
	CAMERA_PIXEL_FORMAT_ENCODED_JPEG = 0x0100,
	CAMERA_PIXEL_FORMAT_ENCODED_H264,

	CAMERA_PIXEL_FORMAT_MAX = 10
} camera_pixel_format_t;

/**
 * @brief The structure type of the camera plances.
 * @since_tizen 3.0
 */
typedef struct camera_planes {
	uint32_t num_planes;
	unsigned char *data[BUFFER_PLANE_MAX];
	uint32_t align_width[BUFFER_PLANE_MAX];
	uint32_t align_height[BUFFER_PLANE_MAX];
	uint32_t size[BUFFER_PLANE_MAX];
} camera_planes_t;
...
typedef struct camera_metadata {
	...
	camera_fraction_t aperture_f_numer;
...
typedef enum camera_command {
	...
	CAMERA_COMMAND_ANTI_HAND_SHAKE,
	CAMERA_COMMAND_FOCUS_AREA,
	CAMERA_COMMAND_HDR_MODE,
...
typedef struct camera_pixel_format_list {
	uint32_t count;
	camera_pixel_format_t format[CAMERA_PIXEL_FORMAT_MAX];
...
typedef struct camera_resolution_list {
	uint32_t count;
	camera_resolution_t resolution[RESOLUTION_COUNT_MAX];
...
/**
 * @brief Called to register for notifications about various message from camera HAL.
 * @since_tizen 3.0
...
/**
 * @brief Called to register for delivering a preview frame when every preview frame is captured from camera device.
 * @since_tizen 3.0
...
/**
 * @brief Called to register for delivering a video frame when every video frame is captured from camera device.
 * @since_tizen 3.0
...
/**
 * @brief Called to get still image data taken by the camera once per frame while capturing.
 * @since_tizen 3.0
...
typedef struct camera_interface {
	...
	int (*set_recording_hint)(void *camera_handle, bool hint);
...
/**
 * @brief Sets the hint of recording.
 * @since_tizen 3.0
 * @remarks The camera HAL can refer this hint to prepare video recording internally.
 * @param[in] camera_handle The handle to the camera HAL
 * @param[in] hint The hint of recording
 * @return @c 0 on success, otherwise a negative error value
 * @retval #CAMERA_ERROR_NONE Successful
 * @retval #CAMERA_ERROR_INVALID_PARAMETER Invalid parameter
 * @retval #CAMERA_ERROR_INVALID_STATE Invalid state
 ...
 * @see camera_get_video_stream_format()
 */
int camera_set_recording_hint(void *camera_handle, bool hint);
 ...
 */
int camera_set_command(void *camera_handle, int command, void *value);
 ...
 * @see camera_get_command()
 */
int camera_get_command(void *camera_handle, int command, void *value);
